# Chapter 1 (excerpt): Two Ways for a Rule-Based System to Fail
...
This is modus ponens. The program checks every possible assignment of `p` and `q`, finds that whenever `impl(p,q)` and `p` are both true, `q` is true as well, and reports the argument valid. No exception exists, so none is found.
...
This looks similar but isn't. It is the classic fallacy of affirming the consequent. The program searches the same small space of assignments and this time finds a counterexample: `p` false, `q` true. Both premises hold under that assignment, `impl(p,q)` is true because a false antecedent makes any implication true, and `q` is true by assumption, but the supposed conclusion `p` is false. The argument is invalid, and the program can show you exactly why, in one line.

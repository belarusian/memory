...
With identical random initialization, differing only in whether the activation function is sigmoid or ReLU (`relu(x) = max(0, x)`, a function this chapter hasn't used yet but is about to matter a great deal):
